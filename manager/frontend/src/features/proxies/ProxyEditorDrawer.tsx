...
const SCHEME_OPTIONS = proxySchemes.map((scheme) => ({
  value: scheme,
  label: scheme === 'direct' ? 'Direct (no proxy)' : scheme.toUpperCase(),
}));
...
export function ProxyEditorDrawer({
  ...
  const onParse = async () => {
    ...
    try {
      ...
      toast({
        title: 'Parsed proxy',
        description: parsed.has_password
          ? 'Password detected — re-enter it below to store it.'
          : undefined,
        tone: 'success',
      });
    } catch (error) {
      toast({ title: 'Could not parse', description: (error as Error).message, tone: 'danger' });
    }
  };
  ...
  return (
    <Drawer
      ...
      description="Reusable across profiles. Passwords are write-only and never returned."
      footer={
        <>
          ...
          >
            {current ? 'Save changes' : 'Create proxy'}
          </Button>
        </>
      }
    >
      <form onSubmit={onSubmit} className="space-y-4">
        <Field label="Label" required error={formState.errors.label?.message}>
          <Input
        ...
        <Field label="Mode" required>
          <Select {...register('scheme')} options={SCHEME_OPTIONS} />
        </Field>
        ...
        {!isDirect && (
          <>
            <Field
              label="Paste &amp; parse"
              hint="Accepts scheme://user:pass@host:port and host:port:user:pass."
            >
            ...
            <div className="grid grid-cols-[1fr_120px] gap-3">
              <Field label="Host" required error={formState.errors.host?.message}>
                <Input
              ...
              <Field label="Port" required error={formState.errors.port?.message}>
                <Input
                  type="number"
                  placeholder="1080"
                  {...register('port')}
            ...
            {scheme === 'socks5' && (
              <p className="rounded-md border border-warning/30 bg-warning/10 p-2.5 text-2xs text-warning">
                Plain SOCKS5 resolves DNS locally. Use SOCKS5H when the proxy should resolve DNS
                remotely.
              </p>
            )}
          </>
        ...
        <div className="flex items-start justify-between gap-4 rounded-md border border-line bg-surface-sunken px-3 py-2.5">
          <div>
            <p className="text-[13px] font-medium text-ink">Test before every launch</p>
            <p className="text-2xs text-ink-faint">
              Quick connectivity check before the browser starts.
            </p>
          </div>
          <Toggle
      ...
      {!isDirect && (
        <div className="mt-5 space-y-3 border-t border-line pt-4">
          <div className="flex items-center gap-2">
            ...
            >
              <Zap className="h-3.5 w-3.5" /> Quick test
            </Button>
            <Button
              variant="secondary"
              size="sm"
            ...
            >
              <Gauge className="h-3.5 w-3.5" /> Full quality test
            </Button>
          </div>
          {!current && <p className="text-2xs text-ink-faint">Save the proxy to enable testing.</p>}
          {current && (
            <>
