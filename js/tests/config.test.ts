...
describe("buildArgs deduplication", () => {
  it("expands consistent preset and lets caller args override it", () => {
    const args = _buildArgsForTest({
      fingerprintPreset: "consistent",
      args: ["--fingerprint-noise=true"],
    });
    expect(args).toContain("--fingerprint-noise=true");
    expect(args).not.toContain("--fingerprint-noise=false");
  });

  it("adds storage quota only for persistent consistent profiles", () => {
    const plain = _buildArgsForTest({ fingerprintPreset: "consistent" });
    const persistent = _buildArgsForTest({
      fingerprintPreset: "consistent",
      userDataDir: "./profile",
    });
    expect(plain).not.toContain("--fingerprint-storage-quota=10240");
    expect(persistent).toContain("--fingerprint-storage-quota=10240");
  });

  it("rejects an unknown fingerprint preset at runtime", () => {
    expect(() => _buildArgsForTest({ fingerprintPreset: "bad" as never })).toThrow(
      /fingerprintPreset/
    );
  });
