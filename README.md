...
### Reusing Playwright for repeated launches

The module-level `launch()` and `launch_async()` functions own an independent
Playwright driver, which is convenient for one browser. For workers that launch
multiple browsers, reuse one driver with a session to avoid paying driver startup
on every launch:

```python
from cloakbrowser import CloakBrowserSession
...
```python
import asyncio
from cloakbrowser import AsyncCloakBrowserSession
...
The session closes any browsers still open when it exits. Closing a browser does
not stop the shared Playwright driver, so later launches remain fast.
...
**Consistent fingerprint preset:** for a returning profile, use a dedicated user-data directory and a fixed fingerprint seed. The preset disables per-render fingerprint noise and presents the stock-Chrome-like 10 GiB storage quota used by persistent profiles. Explicit caller arguments still override the preset.
...
For non-persistent launches, the preset only adds `--fingerprint-noise=false`. The default preset remains unchanged. The equivalent options are `fingerprintPreset: "consistent"` in TypeScript and `FingerprintPreset.Consistent` in .NET.
...
## Proxy quality scanner

`python -m benchmarks.proxy_quality` evaluates a proxy as a separate, opt-in diagnostic. It is not a CAPTCHA solver and it does not click, solve, or retry challenges. Install the optional GeoIP/SOCKS dependencies, provide the proxy only through the environment, and choose an artifact root:
...
# Connectivity and intelligence still use the network; only browser checks are skipped.
$env:PROXY_QUALITY_SKIP_BROWSER = "1"
python -m benchmarks.proxy_quality --output-dir benchmarks/results/proxy-quality

# Separate direct-network Google control; it does not read the proxy or alter its report.
...
The placeholder endpoint above is not a working proxy. Do not put real credentials on the command line: command history and process listings can retain them. The scanner reads `CLOAK_TEST_PROXY` only from the environment and writes a reconstructed, redacted endpoint to its console and JSON reports. Browser scans place a short-lived HTTP relay on an ephemeral `127.0.0.1` port; Chromium receives only that credential-free local endpoint, while the relay authenticates to HTTP, HTTPS, SOCKS5, or SOCKS5H upstreams in process memory and closes with the browser context.

For this scanner relay, `socks5://` resolves destination hostnames locally and therefore intentionally records proxy-DNS as `local`/mismatched. Use `socks5h://` when hostnames must be resolved by the upstream proxy; that remote-DNS form can satisfy the DNS-alignment check. This distinction affects leak evidence, not whether the upstream SOCKS authentication succeeds.

### Reading the verdict

- `type` is one of `mobile`, `residential_or_isp`, `datacenter_or_hosting`, or `unknown`.
- `type_confidence` is `high`, `medium`, or `low`. Two independent structured sources that agree produce `high`; one nonconflicting structured source produces `medium`; heuristic-only evidence or any conflict produces `low`.
- `reputation` is `clean_observed`, `questionable`, `blocked`, or `unknown`.
- `suitable_for_protected_sites` is `yes`, `no`, or `uncertain`.

The summary repeats all four fields above. `classification.evidence` retains only safe ASN, organization, category, carrier, mobile/hosting/privacy flags, and conflicts. Network type and reputation are independent: a residential or mobile IP is not automatically clean, and a datacenter IP is not automatically blocked. `clean_observed` is a timestamped observation of the selected sources and sites, never a universal or permanent claim about an IP.

Unless `PROXY_QUALITY_SKIP_BROWSER=1` is set, the scanner first records field-attributed browser HTTP-exit, WebRTC, timezone, locale, and proxy-DNS evidence, then makes one initial navigation each to a third-party page that embeds Cloudflare Turnstile and to Google Search. The Turnstile result is an observation of that demo only, not a universal Cloudflare reputation verdict. The scanner records page state without clicking or solving a CAPTCHA. Missing identity evidence is `unknown`, an actual mismatch is `questionable`, and a clean verdict requires every identity dimension to align. Skipping browser checks still performs proxy echo requests and may download or refresh intelligence datasets; it is neither deterministic nor offline and cannot produce `clean_observed`.

Each timestamped proxy directory contains `report.json`, `sources.json`, and, when capture succeeds, top-level `cloudflare.png` and `google.png`. PNG metadata is validated and stripped before finalization. The report links to [Pixelscan](https://pixelscan.net/) as a separate deeper fingerprint check; Pixelscan is not run automatically. A direct control is stored under a separate `direct-control/<timestamp>/direct-control.json` tree and is explicitly excluded from proxy evidence.

Downloaded intelligence datasets are cached under the selected output root with source URL, retrieval time, checksum, version/asset details where applicable, and availability. IPsum and the selected FireHOL lists must all be available before their empty matches can support `no_listed_abuse`; an unavailable required source forces an `unknown` conclusion unless stronger block, list-match, consistency, or leak evidence applies. Absence from selected lists is not proof of cleanliness. Attribution and upstream licensing:

- [ipinfo/cli](https://github.com/ipinfo/cli) — Apache-2.0; IPinfo enrichment is optional and its API availability depends on configuration and service access.
- [sapics/ip-location-db](https://github.com/sapics/ip-location-db) — use under the license applicable to the specific downloaded dataset.
- [stamparm/ipsum](https://github.com/stamparm/ipsum) — Unlicense.
- [FireHOL blocklist-ipsets](https://github.com/firehol/blocklist-ipsets) — source-specific licenses; consult the included upstream source information for each list.

See the [scanner design](docs/superpowers/specs/2026-07-21-proxy-quality-scanner-design.md) for its evidence and verdict rules.
