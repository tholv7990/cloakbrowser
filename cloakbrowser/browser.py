...
def _apply_fingerprint_preset(
    args: list[str] | None,
    fingerprint_preset: str | None,
    *,
    persistent: bool,
) -> list[str]:
    """Expand a validated fingerprint preset before caller-argument merging."""
    preset = fingerprint_preset or "default"
    if preset not in {"default", "consistent"}:
        raise ValueError(
            "fingerprint_preset must be 'default' or 'consistent', "
            f"got {fingerprint_preset!r}"
        )
    preset_args: list[str] = []
    if preset == "consistent":
        preset_args.append("--fingerprint-noise=false")
        if persistent:
            preset_args.append("--fingerprint-storage-quota=10240")
    return preset_args + list(args or [])
...
async def _launch_with_playwright_async(
    pw: Any,
    headless: bool = True,
    proxy: str | ProxySettings | None = None,
    args: list[str] | None = None,
    ...
    **kwargs: Any,
    ...
    if humanize:
        ...
        from .human.config import resolve_config
        patch_browser_async(browser, resolve_config(human_preset, human_config))
    return browser
...
def launch(
    ...
    **kwargs: Any,
    ...
    try:
        browser = _launch_with_playwright(
            pw, headless=headless, proxy=proxy, args=args, stealth_args=stealth_args,
...
async def launch_async(  # noqa: C901
    ...
    **kwargs: Any,
    ...
    try:
        browser = await _launch_with_playwright_async(
            pw, headless=headless, proxy=proxy, args=args, stealth_args=stealth_args,
            fingerprint_preset=fingerprint_preset,
            timezone=timezone, locale=locale, geoip=geoip, humanize=humanize,
            human_preset=human_preset, human_config=human_config,
